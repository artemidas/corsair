...
const SELECT_COLS: &str = "id, title, description, severity, resource_type, \
...
pub(crate) async fn list_rules_pool(pool: &SqlitePool) -> Result<Vec<CustomRule>, String> {
...
pub(crate) async fn get_rule_pool(
    pool: &SqlitePool,
    id: &str,
) -> Result<Option<CustomRule>, String> {
...
fn prepare_input(input: CustomRuleInput) -> Result<PreparedInput, String> {
...
fn row_to_rule(row: RuleRow) -> Result<CustomRule, String> {
    let (
        id,
        title,
        description,
...
mod tests {
    ...
    #[test]
    fn builtin_privileged_matches_fixture_pod() {
        let rule = builtin_rules()
            .into_iter()
            .find(|r| r.id == "BUILTIN-001")
            .unwrap();
        let pod = json!({
            "spec": {
                "hostNetwork": true,
        ...
        let leaves = evaluate_field_path(&pod, &rule.field_path);
        assert!(evaluate_operator(
            &leaves,
            rule.operator,
            &rule.expected_value
